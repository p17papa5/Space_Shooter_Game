...
public class BGScrolling : MonoBehaviour
{
    ...
    void Update()
    {
        BG1.position = new Vector3(BG1.position.x - (scrollSpeed * Time.deltaTime), BG1.position.y , BG2.position.z); //smooth frames 
        BG2.position -= new Vector3(scrollSpeed * Time.deltaTime, 0f, 0f);

        if (BG1.position.x < -bgWidth - 1)  //boackground loop
        {
